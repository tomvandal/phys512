"""
Methods and Class to solve PDEs problems
"""
import time
import warnings
...
class Box:
    ...
    Args:
        npix       (int): number of pixel corresponding to the desired
    ...
    def relax(self, maxit=None, tol=1e-2):
        ...
        rhs = Box._get_rhs(self._bc, self._mask)  # 'true' reference
        ax = Box._get_laplacian(self._pot, self._mask, copy=True)  # first lapl
        ...
        for i in range(maxit):
            ...
            r = rhs - Box._get_laplacian(self._pot, self._mask, copy=True)
            rtr = np.sum(r*r)
            if rtr < tol:
    ...
    def cg(self, pot0=None, maxit=None, tol=1e-2):
        ...
        """
        start = time.time()
        self._pot, count = Box._do_cg(self._bc, self._mask, pot0=pot0,
                                      maxit=maxit, tol=tol)
        ttime = time.time() - start
    ...
    def cgres(self, pot0=None, nres=6, maxit=None, tol=1e-2):
        ...
        for i in range(1, nres):
            all_mask[i] = ut.lowres(all_mask[i-1])
            all_bc[i] = ut.lowres(all_bc[i-1])

        # base case (lowest res)
        all_pot[-1], count = Box._do_cg(all_bc[-1], all_mask[-1], pot0=pot0,
                                        maxit=maxit, tol=tol)

        # downgrade resolution nres-1 times from base case (lowest)
        for i in range(nres-2, -1, -1):
            pot0 = ut.upres(all_pot[i+1])
            all_pot[i], counti = Box._do_cg(all_bc[i], all_mask[i], pot0=pot0,
                                            maxit=maxit, tol=tol)
            count += counti
        ttime = time.time() - start
    ...
    def get_rho(self):
        ...
        rho = Box._get_laplacian(self._pot, self._mask, copy=True)[1:-1, 1:-1]
    ...
    def _do_cg(bc, mask, pot0=None, maxit=None, tol=1e-2):
        ...
        if pot0 is None:
            pot0 = bc.copy()
        rhs = Box._get_rhs(bc, mask)  # reference from BC
        ax = Box._get_laplacian(pot0, mask)
        r = rhs - ax
        p = r.copy()
        pot = pot0.copy()
        rtr = np.sum(r*r)
        for i in range(maxit):
            ap = Box._get_laplacian(p, mask)
