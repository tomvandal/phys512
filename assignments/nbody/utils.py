...
def evoframe(frames, model, plot, type):
...
def grid_animation2d(model, niter=50, show=True, savepath=None, figsize=(8, 10),
...
def pts_animation2d(model, niter=50, show=True, savepath=None, figsize=(8, 8),
                    intv=200, ret_fig=False, ret_ani=False, title=None,
                    repeat=False):
    """Animation of 2d nbody model
    Creates an animation of a 2d nbody model with one evolution timestep per
    ...
    if title is not None:
        ax.set_title(title, fontsize=titlesize)
    grid, = ax.plot(model.pos[:, 0], model.pos[:, 1], '*')
    ax.set_xlim([0, model.resol])
    ax.set_ylim([0, model.resol])
    plt.tight_layout()

    anim = animation.FuncAnimation(fig, evoframe, frames=niter, interval=intv,
                                   fargs=(model, grid, 'pts'), repeat=repeat)
    if savepath is not None:
        fformat = savepath.split('.')[-1]
        if fformat != 'gif':
            msg = ('Incorrect file format (.{} instead of .gif). Animation'
